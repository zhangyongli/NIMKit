...
- (void)setupNav
{
    ...
    NSString *img=@"backblack";
    if([self.theme isEqualToString:@"white"]){
        img=@"backwhit";
        [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];
    }else{
         [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];
    }
    [backButton setImage:[UIImage imageNamed:img] forState:UIControlStateNormal];
    [backButton addClick:^{
        [self.parentViewController.parentViewController back:true];
    }];
    UIBarButtonItem *backItem = [[UIBarButtonItem alloc]initWithCustomView:backButton];
     self.navigationItem.leftItemsSupplementBackButton = YES;
    self.navigationItem.leftBarButtonItem = backItem;
    if(self.navBarBgColor){
        [self.navigationController.navigationBar setBarTintColor:[self.navBarBgColor toColor]];
    }
}
...
- (void)setUpTitleView
{
    ...
    if (!titleView || ![titleView isKindOfClass:[NIMKitTitleView class]])
    {
        ...
        titleView.titleLabel.textColor=[color toColor];
        self.subTitleLabel.textColor=[color toColor];
    }
